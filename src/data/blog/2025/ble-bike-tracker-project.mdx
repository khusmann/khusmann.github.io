...
The connector works, but the plastic chassis of the bike was a little too thick for it, so it doesn't work with wider USB-C connectors. For other project I'll probably look for other ways to panel-mount USB-C input.

### 3. A Patch Into The Reed Switch

The line for the reed switch was wasy to locate, and I easily spliced my own cable into it. I kept it connected to the built-in LCD indicator on the bike, so that cadence and other info could still be displayed there. I handled level shifting with a few diodes / resistors that hooked up on the proto board in the microcontroller chassis.
